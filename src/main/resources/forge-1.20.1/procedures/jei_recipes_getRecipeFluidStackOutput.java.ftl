    ...
    new Object() {
        public FluidStack getResult(
            ...
            if(world instanceof Level level) {
                ...
                recipes.addAll(manager.getAllRecipesFor(${recipeName}Recipe.Type.INSTANCE));
            }
            ...
            for(${recipeName}Recipe recipe : recipes) {
                boolean _itemMatch = true
                    <#list input_list$entry as entry>
                        ...
                        <#if typeArray[i] == "MCItem">
                            && JEIUtils.validate(${nameArray[i]}ItemInput, recipe.${nameArray[i]}ItemInput())
                        </#if>
                    </#list>
                ;
                boolean _fluidMatch = true
                    <#list input_list$entry as entry>
                        ...
                        <#if typeArray[i] == "FluidStack">
                            && JEIUtils.validate(${nameArray[i]}FluidInput, recipe.${nameArray[i]}FluidInput())
                        </#if>
                    </#list>
                ;
                boolean _logicMatch = true
                    <#list input_list$entry as entry>
                        ...
                        <#if typeArray[i] == "Boolean">
                            && JEIUtils.validate(${nameArray[i]}LogicInput, ${nameArray[i]}LogicInput())
                        </#if>
                    </#list>
                ;
                boolean _numberMatch = true
                    <#list input_list$entry as entry>
                        ...
                        <#if typeArray[i] == "Number">
                            && JEIUtils.validate(${nameArray[i]}NumberInput, recipe.${nameArray[i]}NumberInput())
                        </#if>
                    </#list>
                ;
                boolean _textMatch = true
                    <#list input_list$entry as entry>
                        ...
                        <#if typeArray[i] == "String">
                            && JEIUtils.validate(${nameArray[i]}TextInput, recipe.${nameArray[i]}TextInput())
                        </#if>
                    </#list>
                ;

                if(_itemMatch && _fluidMatch && _logicMatch && _numberMatch && _textMatch) {
                    result = recipe.getFluidStackResult("${field$name}");
                    ...
                    if(recipeInputNotConsumed) {
                        <#list input_list$entry as entry>
                            ...
                            <#if consumeArray[i] == "TRUE">
                                <#if typeArray[i] == "MCItem">
                                    ${nameArray[i]}ItemInput.shrink(recipe.${nameArray[i]}ItemInput().getCount());
                                <#elseif typeArray[i] == "FluidStack">
