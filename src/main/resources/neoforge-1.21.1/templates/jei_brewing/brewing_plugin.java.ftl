        ...
        <#if brewingRecipes??>
		    ...
		    <#list brewingRecipes as recipe>
                <#if recipe.brewingIngredientStack.getUnmappedValue().startsWith("TAG:")>
                    ingredientStack = new ArrayList<ItemStack>(BuiltInRegistries.ITEM.getOrCreateTag(ItemTags.create(new ResourceLocation("${recipe.brewingIngredientStack?replace("TAG:","")}"))).stream().map(item -> new ItemStack((Item) item.value())).collect(Collectors.toCollection(ArrayList::new)));
                <#else>
