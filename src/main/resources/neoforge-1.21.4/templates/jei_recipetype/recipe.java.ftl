    ...
    public static class Serializer implements RecipeSerializer<${name}Recipe> {
        ...
        private static final MapCodec<${name}Recipe> CODEC = RecordCodecBuilder.mapCodec(
            builder -> builder.group(
                        ItemStack.STRICT_CODEC.fieldOf("output").forGetter(TestRecipe::getResultItem),
                        Ingredient.CODEC
                            .listOf()
                            .fieldOf("ingredients")
                            .flatXmap(
                                ingredients -> {
                                    ...
                                    if (aingredient.length == 0) {
                                        ...
                                        return DataResult.success(NonNullList.of(Ingredient.of(), aingredient));
                                    }
                                },
                                DataResult::success
                            )
        ...
        private static ${name}Recipe fromNetwork(RegistryFriendlyByteBuf buf) {
            NonNullList<Ingredient> inputs = NonNullList.withSize(buf.readVarInt(), Ingredient.of());
